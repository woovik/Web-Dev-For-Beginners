...
- **Имя переменной**, вы сможете выбрать любое имя на свое усмотрение.
...
1. **Присвоить значение**. Запишите значение переменной с помощью оператора `=`, за которым следует ожидаемое значение.
...
Константы аналогичны переменным, но с некоторыми отличиями:
...
- **Ссылка не может быть изменена**. Ссылка на константу не может быть изменена после инициализации, иначе при выполнении кода возникнет ошибка. Давайте рассмотрим два примера:
  ...
  - **Значение объекта не защищено**. Следующее допускается:
    ...
    Выше вы меняете значение объекта, но не саму ссылку, что делает его доступным.

  > Обратите внимание, что ссылка у `const` защищена от повторного использования. Однако значение не является _неизменным_ и может измениться, особенно если это такая сложная конструкция как объект.
...
✅ Типы данных также называют примитивными данными JavaScript, поскольку они являются типами данных самого низкого уровня, предоставляемые языком. Существует 6 примитивных типов данных: string, number, bigint, boolean, undefined, and symbol. Потратьте минуту, чтобы представить, что может значить каждый из этих примитивов. Что такое `зебра`? Как насчет `0`? `true`?
...
✅ Попробуйте! Выполните арифмитическую операцию прямо в консоли браузера. Результат вас удивил?
...
✅ Почему в JavaScript `1 + 1 = 2`, но `'1' + '1' = 11?` Подумайте об этом. Что насчет `'1' + 1`?

**Литералы шаблонов** это еще один способ форматирования строк, за исключением того, что вместо кавычек используется обратная кавычка. Все, что не является обычным текстом, должно быть внутри заполнителя `${ }`. Сюда подойдут любые переменные, которые могут быть строками.
.
